...
Сборка от 6 августа 2026 года добавляет безопасную основу постоянной работы:

- отдельный `ops-watcher` в Docker с циклом 60 секунд;
- детерминированные правила доступности, heartbeat, очередей, источников,
  Telegram-доставки и AI-бюджета;
- ежедневный HTTP smoke ключевых публичных маршрутов;
- дедупликацию, историю и автоматическое закрытие восстановившихся findings;
- Telegram-алерты P0/P1 только связанным аккаунтам администраторов;
- `/admin/findings` с фильтрами, историей, ручным triage и kill switch;
- закрытый `/api/admin/ops-snapshot`, доступный только после admin auth;
- R2 allowlist и proposals, которые остаются выключенными в shadow mode и не
  могут активироваться ранее семи дней наблюдения;
- GitHub quality gate для lint, typecheck, ops/admin/security tests и build.

Production rollout начинает новый семидневный shadow-период. Переход в
`recommend`, а затем в `limited` не выполняется автоматически.
