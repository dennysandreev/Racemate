...
import type { DriverChartPoint, DriverCumulativePointsSeries, DriverProfile } from "@/types/racemate";
...
export function DriverCumulativePointsChart({ profile }: { profile: DriverProfile }) {
  ...
  const series = selectedSeries
    ...
    : [{ driverId: profile.id, driver: profile.fullName, points: rounds, teamColor: profile.team.color ?? undefined }];
  ...
  function handlePointerMove(event: PointerEvent<SVGPolylineElement>, item: DriverCumulativePointsSeries) {
    const color = item.driverId === profile.id
      ? profile.team.color ?? "oklch(var(--primary))"
      ...
      <div className="grid gap-4 p-4 sm:p-5">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-2 text-foreground">
            <span className="size-2 rounded-full" style={{ backgroundColor: profile.team.color ?? "oklch(var(--primary))" }} />
            {profile.fullName}
          </span>
        ...
        {rounds.length ? (
          <div className="relative h-64 min-w-0 select-none sm:h-72">
            <div className="absolute bottom-8 left-10 right-3 top-2">
              <svg
                aria-label={`Накопленные очки ${profile.fullName} и лидеров чемпионата`}
              ...
              >
                {yTicks.map((value) => {
                  ...
                  return <line key={value} stroke="currentColor" strokeOpacity="0.14" strokeWidth="0.45" x1="0" x2="100" y1={y} y2={y} />;
                })}
                ...
                {series.map((item) => {
                  const isSelected = item.driverId === profile.id;
                  const isHovered = hovered?.driverId === item.driverId;
                  ...
                  const color = isSelected
                    ...
                    : isHovered
                      ? item.teamColor ?? "oklch(var(--primary))"
                      : "currentColor";
                  const opacity = isHovered || (!hasHover && isSelected) ? 1 : hasHover ? 0.18 : isSelected ? 1 : 0.32;

                  return (
                    <g key={item.driverId} opacity={opacity}>
                      <polyline
                        fill="none"
                        ...
                        strokeWidth={isHovered || isSelected ? "2.5" : "1.05"}
                      />
                      {item.points.map((point, index, points) => (
                        <circle
                          ...
                          fill={color}
                          key={`${item.driverId}-${point.round}`}
                          r={isHovered ? "1.75" : isSelected ? "1.35" : "0.75"}
                          stroke={isSelected || isHovered ? "oklch(var(--background))" : "none"}
                          strokeWidth="0.5"
                        />
                      ))}
                      <polyline
