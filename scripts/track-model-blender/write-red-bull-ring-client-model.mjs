...
import { utm33nFromWgs84 } from "./export-red-bull-ring-track-data.mjs";

const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const projectRoot = path.resolve(scriptDirectory, "../..");

export async function writeRedBullRingClientModel({
  configPath = path.join(projectRoot, ".track-model-build/red-bull-ring.json"),
  metadataPath = path.join(projectRoot, "public/f1/tracks/3d/red-bull-ring-metadata.json"),
  outputPath = path.join(projectRoot, "src/data/red-bull-ring-model.ts"),
} = {}) {
  const preparedDirectory = path.join(projectRoot, ".track-model-build/red-bull-ring-prepared");
  const [config, metadata, rasterMetadata, dtmBuffer] = await Promise.all([
    ...
    readFile(path.join(preparedDirectory, "styria-dtm.f32le")),
  ]);
  ...
  const pitCenterline = pitWay.geometry.map(({ lat, lon }) => utm33nFromWgs84(lat, lon));
...
export const RED_BULL_RING_MODEL = ${JSON.stringify({
...
export const RED_BULL_RING_TERRAIN = ${JSON.stringify({
...
export const RED_BULL_RING_REPLAY_PATH = ${JSON.stringify({
...
export const RED_BULL_RING_TRACK_MODEL = ${JSON.stringify({
    aliases: [
      "red bull ring",
      "ред булл ринг",
      "spielberg",
    ...
    webgl: {
      ...
      camera: { fitHeight: 2_140, fitWidth: 2_510, lookAtY: 34, radius: 3_450 },
      elevationDatumLabel: "м ALS",
      previewPath: "/f1/tracks/3d/red-bull-ring-preview.webp",
      turnCount: 10,
    },
  }, null, 2)
    .replace('"__MODEL__"', "RED_BULL_RING_MODEL")
    .replace('"__TERRAIN__"', "RED_BULL_RING_TERRAIN")} satisfies TrackModelDefinition;
`;
