...
test("scrubSentryEvent removes user data, credentials, ids, and URL queries", () => {
  const event = scrubSentryEvent({
    breadcrumbs: [{
      data: { provider_id: "provider-123", safe: "kept" },
      message: "Request for fan@example.com used Bearer private-token",
    }],
    extra: { chat_id: 123456, nested: { api_key: "private-key" } },
    request: {
    ...
    user: { email: "fan@example.com", id: "user-123", ip_address: "127.0.0.1" },
  });
  ...
  assert.equal(event.breadcrumbs[0].message, "Request for [Filtered] used Bearer [Filtered]");
});
...
test("Sentry helpers validate sampling and use the shared release SHA", () => {
  ...
  assert.equal(resolveSentryRelease({ RACESIDE_RELEASE_SHA: "release-123" }), "release-123");
});
